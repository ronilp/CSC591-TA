...
def main():
    ...
    if method == "d2v":
        ...
        filename = os.path.join(path_to_data, 'train_pos_vec_d2v.txt')
        # pickle.dump(train_pos_vec, open(filename, 'wb'))
        train_pos_vec = pickle.load(open(filename, 'rb'))
        filename = os.path.join(path_to_data, 'train_neg_vec_d2v.txt')
        # pickle.dump(train_neg_vec, open(filename, 'wb'))
        train_neg_vec = pickle.load(open(filename, 'rb'))
        filename = os.path.join(path_to_data, 'test_pos_vec_d2v.txt')
        # pickle.dump(test_pos_vec, open(filename, 'wb'))
        test_pos_vec = pickle.load(open(filename, 'rb'))
        filename = os.path.join(path_to_data, 'test_neg_vec_d2v.txt')
        # pickle.dump(test_neg_vec, open(filename, 'wb'))
        test_neg_vec = pickle.load(open(filename, 'rb'))

        # nb_model, lr_model = build_models_DOC(train_pos_vec, train_neg_vec)
        filename = os.path.join(path_to_data, 'nb_model_d2v.sav')
        # pickle.dump(nb_model, open(filename, 'wb'))
        nb_model = pickle.load(open(filename, 'rb'))
        filename = os.path.join(path_to_data, 'lr_model_d2v.sav')
        # pickle.dump(lr_model, open(filename, 'wb'))
        lr_model = pickle.load(open(filename, 'rb'))
    ...
    if method == "w2v":
        train_pos_vec, train_neg_vec, test_pos_vec, test_neg_vec = feature_vecs_DOC_W2V(train_pos, train_neg, test_pos,
                                                                                        test_neg)
        filename = os.path.join(path_to_data, 'train_pos_vec_w2v.txt')
        pickle.dump(train_pos_vec, open(filename, 'wb'))
        # train_pos_vec = pickle.load(open(filename, 'rb'))
        filename = os.path.join(path_to_data, 'train_neg_vec_w2v.txt')
        pickle.dump(train_neg_vec, open(filename, 'wb'))
        # train_neg_vec = pickle.load(open(filename, 'rb'))
        filename = os.path.join(path_to_data, 'test_pos_vec_w2v.txt')
        pickle.dump(test_pos_vec, open(filename, 'wb'))
        # test_pos_vec = pickle.load(open(filename, 'rb'))
        filename = os.path.join(path_to_data, 'test_neg_vec_w2v.txt')
        pickle.dump(test_neg_vec, open(filename, 'wb'))
        # test_neg_vec = pickle.load(open(filename, 'rb'))

        nb_model, lr_model = build_models_DOC_W2V(train_pos_vec, train_neg_vec)
        filename = os.path.join(path_to_data, 'nb_model_w2v.sav')
        pickle.dump(nb_model, open(filename, 'wb'))
        filename = os.path.join(path_to_data, 'lr_model_w2v.sav')
        pickle.dump(lr_model, open(filename, 'wb'))
...
def load_data(path_to_dir):
    ...
    test_pos = []
    test_neg = []
    with open(os.path.join(path_to_dir, "train-pos.txt"), "r") as f:
        for i, line in enumerate(f):
            words = [w.lower() for w in line.strip().split() if len(w) >= 3]
            if (len(words) == 0): continue
            train_pos.append(words)
    with open(os.path.join(path_to_dir, "train-neg.txt"), "r") as f:
        for line in f:
            words = [w.lower() for w in line.strip().split() if len(w) >= 3]
            if (len(words) == 0): continue
            train_neg.append(words)
    with open(os.path.join(path_to_dir, "test-pos.txt"), "r") as f:
        for line in f:
            words = [w.lower() for w in line.strip().split() if len(w) >= 3]
            if (len(words) == 0): continue
            test_pos.append(words)
    with open(os.path.join(path_to_dir, "test-neg.txt"), "r") as f:
        for line in f:
            words = [w.lower() for w in line.strip().split() if len(w) >= 3]
            if (len(words) == 0): continue
...
def feature_vecs_DOC(train_pos, train_neg, test_pos, test_neg):
    ...
    # Use the docvecs attribute to extract the feature vectors for the training and test data
    # YOUR CODE HERE
